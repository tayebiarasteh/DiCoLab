...
%%
function SER = MMSE(M,c,N_r,N_t)

SER_mean = []; % average SER
SER_vec = [];

for SNR_db = -10:5:50
    x= GetQAM(c,c,M); % get the constellation
    ...
    for h=1:10000 
        ...
        y = H*x + noise ;
        y= inv((H'*H + (1/SNR_lin)*eye(size(H))))*(H')*y;
...
semilogy(-10:5:50, SER_mean,'g'); xlabel('SNR'); ylabel('SER');  title('MIMO MMSE'); 
legend('MMSE N_r = N_t = 2'); grid on;
hold on;
